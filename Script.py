...
class ChatClient:
    def __init__(self, username, room):
        ...
        self.chat_area = scrolledtext.ScrolledText(chat_frame, wrap=tk.WORD, state='disabled', bg='#1E1E1E', fg='#FFFFFF', font=('Arial', 12))
      ...
      #  self.emoji_button.pack(side=tk.RIGHT, padx=5)
        ...
        self.send_system_message(f"{self.username} has joined the chat. Welcome {self.username}! My Name is Olive, Im here to help with any inquiries you might have Select from an option below,")
        # self.chat_area.pack_propagate(False)


        ChatBotOptions = ["Help with an order","Delivery estimate","something else"]

        for x in range (0,3):

         
          self.option_button = tk.Button(chat_frame,text= ChatBotOptions[x],  command=lambda x=x: self.Ai_chatbot_response(x),bg='#3E3E3E', fg='#FFFFFF', font=('Arial', 12))
          self.option_button.index = x
          self.option_button.pack(side=tk.TOP, pady= 10, expand= False)
    ...
    def  Ai_chatbot_response(self, index):

     if index == 0:
        response = "Response for option 0"
     elif index == 1:
